...
class Model(object):
    ...
    def evaluate(self, x):
        curA = x.T # tranposes input, not sure what the purpose of this is exactly (convert to column vector perhaps?)
        a = [curA] # initialize the list of all inputs layer 0 is the input layer
        z = []
        for layer in self.layers: 
            z.append(layer.z(curA)) # calculate the output of the next layer.
            curA = layer.a(z[-1]) # input to the next layer is the sigma(a_i) before so that is that
            a.append(curA) # add the next thing
        yp = a.pop() # remove the output of the nextwork as requested
        return yp, a, z
    ...
    def compute_grad(self, x, y):
        ...
        # Compute the error
        d = self.cost.dx(y.T, yp) # this is holding the current gradient dMSE/dz_i+1 that we will use (backward flow)
        grad = []

        # Backpropogate the error
        idx = len(self.layers)

        # need to go over the layers in reverse order from the top-most layer to the very first layer
        for i, (layer, curZ) in enumerate(zip(reversed(self.layers),reversed(z))):
            idx = idx - 1
            grad.insert(0,np.multiply(d,layer.dx(curZ))) # d

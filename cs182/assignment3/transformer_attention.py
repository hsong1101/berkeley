...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        if len(keys.shape) == 3:
          trans_keys = tf.transpose(keys, [0,2,1])
        else:
          trans_keys = tf.transpose(keys, [0,1,3,2])

        similarity = tf.matmul(queries, trans_keys) / tf.sqrt(key_dim) # Compute the similarity according to the QKV formula
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        batch_size, tensorlen = tf.shape(tensor)[0], tensor.shape[1]

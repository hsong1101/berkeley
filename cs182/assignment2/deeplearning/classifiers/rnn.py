...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            out_hidden, cache_hidden = rnn_forward(out_word, out_proj, Wx, Wh, b)

        elif self.cell_type == 'lstm':
            out_hidden, cache_hidden = lstm_forward(out_word, out_proj, Wx, Wh, b)
            
            

        out_temp, cache_temp = temporal_affine_forward(out_hidden, W_vocab, b_vocab)
        loss, dscores = temporal_softmax_loss(out_temp, captions_out, mask)
        
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, cache_temp)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
          ...
          id = np.squeeze(np.argmax(score, axis=2))

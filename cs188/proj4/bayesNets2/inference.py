...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        preprocessed = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for i in eliminationOrder:
            joinedFactor = joinFactorsByVariable(preprocessed, i)
            preprocessed = joinedFactor[0]

            if len(joinedFactor[1].unconditionedVariables()) == 1:
                continue

            preprocessed.append(eliminate(joinedFactor[1], i))

        return normalize(joinFactors(preprocessed))

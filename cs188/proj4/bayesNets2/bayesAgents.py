...
def constructBayesNet(gameState):
    ...
    variableDomainsDict = {X_POS_VAR:X_POS_VALS, Y_POS_VAR:Y_POS_VALS, GHOST_HOUSE_VAR:HOUSE_VALS, FOOD_HOUSE_VAR:HOUSE_VALS}


    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obs = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obs)
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        ...
        if housePos == bottomLeftPos:
            loc = BOTTOM_LEFT_VAL
        if housePos == topLeftPos:
            loc = TOP_LEFT_VAL
        if housePos == bottomRightPos:
            loc = BOTTOM_RIGHT_VAL
        if housePos == topRightPos:
            loc = TOP_RIGHT_VAL

        for walls in gameState.getHouseWalls(housePos):
            obs = OBS_VAR_TEMPLATE % walls

            obsFactor = bn.Factor([obs], HOUSE_VARS, bayesNet.variableDomainsDict())

            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if obs in assignment:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        values.append(factor.getProbability(assignment))
    topLeftPos, topRightPos, bottomLeftPos, bottomRightPos = values
    highest = max(values)

    if bottomLeftPos == highest:
        return {FOOD_HOUSE_VAR: BOTTOM_LEFT_VAL}
    if topLeftPos == highest:
        return {FOOD_HOUSE_VAR: TOP_LEFT_VAL}
    if bottomRightPos == highest:
        return {FOOD_HOUSE_VAR: BOTTOM_RIGHT_VAL}
    if topRightPos == highest:
        return {FOOD_HOUSE_VAR: TOP_RIGHT_VAL}
...
class VPIAgent(BayesAgent):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            ...
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                leftExpectedValue += factor.getProbability(assignment) * GHOST_COLLISION_REWARD
                rightExpectedValue += factor.getProbability(assignment) * WON_GAME_REWARD
                
            elif assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExpectedValue += factor.getProbability(assignment) * WON_GAME_REWARD
                rightExpectedValue += factor.getProbability(assignment) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        temp = self.getExplorationProbsAndOutcomes(evidence)
        for x in temp:
            temp2 = self.computeEnterValues(x[1], enterEliminationOrder)

            if temp2[0] > temp2[1]:
                expectedValue += temp2[0] * x[0]
            else:
                expectedValue += temp2[1] * x[0]

...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    True
    """
    row_win, col_win = False, False

    row, new_board = make_move(board, max_rows, max_cols, col, player)

    for x in range(max_rows):
        row_win = check_win_row(new_board, max_rows, max_cols, num_connect, x, player)

    for y in range(max_cols):
        col_win = check_win_column(new_board, max_rows, max_cols, num_connect, x, player)

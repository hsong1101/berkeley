...
CREATE TABLE schedule AS
  with
    transfer(n, d, a, p) as (
        select 1, departure, arrival, price from flights union
        select n+1, d, a, p from transfer where n < 3 order by p
    )
  select d || ', ' || a from transfer where d = "SFO" or a = "PDX";
...
CREATE TABLE healthiest_meats AS
  SELECT "tofurky|1400" union select "turkey|1900";

CREATE TABLE average_prices AS
  SELECT "computer|109.09" union select "games|349.99" union select "phone|89.99";

...
def replace_leaf(t, old, new):
    ...
    def fn(tree, old, new):
        ...
        for branch in branches(tree):
            if label(branch) == old and is_leaf(branch):
                branch[0] = new
            fn(branch, old, new)

        return tree
    
    return fn(temp, old, new)
...
def multiple_references_explanation():
    return """It's true. I don't the the correct reason why but the larger the difference in intervals, the bigger different reference gets"""
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme <= upper_bound(x):
        return interval(min(lower,upper,ex), max(lower,upper,ex))
